import time
#from arraySorted import hybridMergeSort
from Hibrido_teste import hybridMergeSort
...
if not lista:
    ...
    # Executar o Hybrid Sort várias vezes
    for i in range(num_execucoes):
        ...
        hybridMergeSort(lista_copia, 0, len(lista_copia) - 1)  # Executar o Hybrid Sort
        ...
        tempos_hybrid.append(tempo_execucao)
   ...
   # print("\nLista ordenada da última execução:", lista_copia)
    ...
    tempo_medio_bubble_sort = sum(tempos_hybrid) / num_execucoes
    print(f"\nTempo médio do Hybrid Sort: {tempo_medio_bubble_sort:.4f} ms")
    
    # Exibir o tempo em um gráfico usando Seaborn
    sns.set_theme(style="whitegrid")
    sns.lineplot(x=[f"{i+1}" for i in range(num_execucoes)], y=tempos_hybrid, color="Purple")
    plt.xlabel('Execuções')
    plt.ylabel('Tempo de Execução (ms)')
    plt.title(f'Tempos de execução do hybridSort (em {num_execucoes} execuções)')
    plt.show()
